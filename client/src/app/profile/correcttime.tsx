...
const ProfilePage = () => {
  ...
  const sortedTasks = filteredTasks.slice().sort((a, b) => {
    const timeA = calculateTimeSpent(a, filteredTasks);
    const timeB = calculateTimeSpent(b, filteredTasks);
    return timeB - timeA;
  });
  ...
  const calculateTotalTimeSpent = () => {
    return sortedTasks.reduce((total, task) => {
      const timeSpent = calculateTimeSpent(task, sortedTasks);
                ...
                <tr key={task.id} className="border">
                  ...
                  <td className="border p-2">
                    {task.startDate && task.dueDate
                      ? calculateTimeSpent(task, sortedTasks).toFixed(2)
                      : "N/A"}
                  </td>
                </tr>
              ))
